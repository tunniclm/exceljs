...
class WorksheetWriter {
  constructor(options) {
    ...
    // queue for rows committed to be written to the stream
    // so that they can be submitted synchronously without causing
    // problems
    this._rowCommitQueue = [];
    // when the commit queue is being processed, this is a promise
    // that will resolve when processing has been completed and the
    // queue is empty (at which point it is cleared to null)
    this._rowCommitQueuePromise = null;
  ...
  async commit() {
    if (this.committed || this._rows === null) {
      // do not commit the worksheet more than once
      return;
    }
    // commit all rows
    for (const cRow of this._rows) {
      if (cRow) {
        // queue the row to be written to the stream
        this._submitRowForCommit(cRow);
      }
    }

    // we _cannot_ accept new rows from now on
    // NOTE: we are also using this as an indicator that we have started
    //       to commit this worksheet. As such it needs to happen before
    //       the first await in this function (not all callers will necessarily
    //       await calling us since this function was not async before).
    this._rows = null;

    // wait for all the queued rows to be written to the stream before we
    // proceed to write the rest of the file to stream
    await this._rowCommitQueuePromise;
  ...
  _submitRowForCommit(cRow) {
    return new Promise(resolve => {
      // add this row to the commit queue and store a promise resolve callback
      // that will trigger this Promise to resolve when the row has been
      // written to the stream
      this._rowCommitQueue.push({rowCommitCallback: resolve, row: cRow});

      // start commit queue processing if it is not already running
      if (!this._rowCommitQueuePromise) {
        this._rowCommitQueuePromise = this._startRowCommitQueueLoop();
      }
    });
  }

  async _startRowCommitQueueLoop() {
    // write the queued rows to the stream waiting for it to drain as required
    // to properly support backpressure
    while (this._rowCommitQueue.length) {
      const {rowCommitCallback, row} = this._rowCommitQueue.shift();
      // write the row to the stream and wait if the stream needs to drain
      await new Promise(resolve => this._writeRow(row, resolve)); // eslint-disable-line no-await-in-loop
      // resolve the row commit promise
      rowCommitCallback();
    }
    // mark the end of commit queue processing
    this._rowCommitQueuePromise = null;
  }
  ...
  async _commitRow(cRow) {
    ...
    let found = false;
    let lastRowPromise = null;
    while (this._rows.length && !found) {
      const row = this._rows.shift();
      this._rowZero++;
      if (row) {
        lastRowPromise = this._submitRowForCommit(row);
        found = row.number === cRow.number;
        this._rowZero = row.number + 1;
      }
    }
    return lastRowPromise || Promise.resolve();
  }
  ...
  _writeRow(row, callback) {
    ...
    if (row.hasValues || row.height) {
      ...
      const ready = this.stream.write(xform.row.toXml(model));
      if (ready) {
        callback();
      } else {
