...
const NoBatchStream = require('../../utils/no-batch-stream');
const BatchStream = require('../../utils/batch-stream');
...
class WorkbookWriter {
  constructor(options) {
    ...
    this.writeStreamType = options.writeStreamType;
    if (!['stream-buf', 'no-batch', 'batch'].includes(this.writeStreamType)) {
      this.writeStreamType = 'batch';
    }
  ...
  _openStream(path) {
    const makeStream = (type) => {
      if (type === 'stream-buf') return new StreamBuf({bufSize: 65536, batch: true});
      if (type === 'no-batch') return new NoBatchStream();
      return new BatchStream();
    };
    const stream = makeStream(this.writeStreamType);
